...
public class DropCropsGoal extends Goal {
    ...
    public DropCropsGoal(PathfinderMob mob, double speedModifier, int distanceToPoi) {
        this.mob = mob;
            ...
            if (!serverlevel.isCloseToVillage(blockpos, 6)) {
                ...
                Vec3 vec3 = LandRandomPos.getPos(this.mob, 15, 7, (p_217751_) -> {
                    if (!serverlevel.isVillage(p_217751_)) {
                        ...
                        Optional<BlockPos> optional1 = serverlevel.getPoiManager().find((p_217758_) -> p_217758_.is(PoiTypeTags.VILLAGE), this::hasNotVisited, p_217751_, 10, Occupancy.IS_OCCUPIED);
                        return (Double)optional1.map((p_217754_) -> -p_217754_.distSqr(blockpos)).orElse(Double.NEGATIVE_INFINITY);
                    }
                });
                if (vec3 == null) {
                    return false;
                } else {
                    Optional<BlockPos> optional = serverlevel.getPoiManager().find((p_217756_) -> p_217756_.is(PoiTypeTags.VILLAGE), this::hasNotVisited, BlockPos.containing(vec3), 10, Occupancy.IS_OCCUPIED);
                    if (optional.isEmpty()) {
                        return false;
                    } else {
